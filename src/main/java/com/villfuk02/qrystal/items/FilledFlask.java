    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        if(stack.hasTag() && stack.getTag().contains("fluid")) {
            ...
            if(ForgeRegistries.FLUIDS.containsKey(fluid)) {
                tooltip.add(new TranslationTextComponent(ForgeRegistries.FLUIDS.getValue(fluid).getAttributes().getTranslationKey()).appendSibling(
                        new StringTextComponent(" " + amt + "mB")).applyTextStyle(TextFormatting.AQUA));
                if(FluidTierManager.solvents.containsKey(fluid))
                    tooltip.add(new TranslationTextComponent("qrystal.tier." + FluidTierManager.solvents.get(fluid).getKey()).appendSibling(
                            new TranslationTextComponent("qrystal.solvent_tier")).applyTextStyle(TextFormatting.GOLD));
            } else {
