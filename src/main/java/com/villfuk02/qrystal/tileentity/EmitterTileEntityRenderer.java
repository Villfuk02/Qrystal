...
public class EmitterTileEntityRenderer extends TileEntityRenderer<EmitterTileEntity> {
    public static final ResourceLocation TEXTURE_BEACON_BEAM = new ResourceLocation("textures/entity/beacon_beam.png");
    static float[][] tierColors = new float[][]{{1, 0, 0}, {1, 0.4f, 0}, {0.4f, 1, 0.6f}};
    
    public EmitterTileEntityRenderer(TileEntityRendererDispatcher p_i226006_1_) {
        super(p_i226006_1_);
        
    }
    
    @Override
    public void render(EmitterTileEntity tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
        long i = tileEntityIn.getWorld().getGameTime();
        byte range = tileEntityIn.range;
        if(range > 0 && (isPowerItem(Minecraft.getInstance().player.getHeldItem(Hand.MAIN_HAND)) || isPowerItem(Minecraft.getInstance().player.getHeldItem(Hand.MAIN_HAND))))
            renderBeamSegment(matrixStackIn, bufferIn, partialTicks, i, -0.5f, range + 1, EmitterTileEntity.getTier(tileEntityIn.getBlockState()),
                              tileEntityIn.getBlockState().get(DirectionalBlock.FACING).getRotation());
    }
    
    private boolean isPowerItem(ItemStack heldItem) {
        if(heldItem.getItem() instanceof BlockItem) {
            Block b = ((BlockItem)heldItem.getItem()).getBlock();
            return b instanceof EmitterBlock || b instanceof ReceiverBlock || b instanceof CutterBlock || b instanceof PoweredEvaporatorBlock || b instanceof UltimateEvaporatorBlock ||
                    b instanceof PoweredFluidMixerBlock || b instanceof UltimateFluidMixerBlock;
        }
        return false;
    }
    
    private static void renderBeamSegment(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, float partialTicks, long totalWorldTime, float yOffset, int height, int tier, Quaternion rotation) {
        renderBeamSegment(matrixStackIn, bufferIn, TEXTURE_BEACON_BEAM, partialTicks, 1.0F, totalWorldTime, yOffset, height, tierColors[tier], 0.04F + 0.01f * tier, 0.07F + 0.01f * tier, rotation);
    }
    ...
    public static void renderBeamSegment(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, ResourceLocation textureLocation, float partialTicks, float textureScale, long totalWorldTime, float yOffset,
                                         int height, float[] colors, float beamRadius, float glowRadius, Quaternion rotation) {
    ...
    @Override
    public boolean isGlobalRenderer(EmitterTileEntity te) {
        return true;
    }
}
