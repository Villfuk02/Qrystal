...
public class LaserCutterTileEntity extends CutterTileEntity {
    public LaserCutterTileEntity() {
        super(ModTileEntityTypes.LASER_CUTTER, 6,
              s -> s.getItem() instanceof Crystal && ((Crystal)s.getItem()).size == CrystalUtil.Size.LARGE && ((Crystal)s.getItem()).tier >= 6 && s.hasTag() && s.getTag().contains("material") &&
                      s.getTag().getString("material").equals(CrystalUtil.Color.QALB.toString()), s -> ((Crystal)s.getItem()).tier - 6, (byte)3, RecipeUtil.CuttingType.LASER, ModBlocks.LASER_CUTTER);
    }
}
