...
public class FilledFlaskColorHandler implements IItemColor {
    ...
    public int getColor(ItemStack stack, int tintIndex) {
        if(tintIndex != 1)
            return 0xFFFFFF;
        if(!stack.hasTag() || !stack.getTag().contains("fluid", Constants.NBT.TAG_STRING) || !ForgeRegistries.FLUIDS.containsKey(new ResourceLocation(stack.getTag().getString("fluid"))))
            return 0xFF00FF;
    ...
    public static int[] getColor(ResourceLocation fluid) {
        ...
        int[] txc = ColorUtils.unwrapRGBA(
                sprite.getPixelRGBA(0, (int)(Minecraft.getInstance().world.getGameTime() / 41) % sprite.getWidth(), (int)(Minecraft.getInstance().world.getGameTime() / 29) % sprite.getHeight()));
