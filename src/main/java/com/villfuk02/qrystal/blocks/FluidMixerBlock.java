...
public abstract class FluidMixerBlock extends HorizontalBlock {
    ...
    public void onReplaced(BlockState oldState, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if(oldState.getBlock() != newState.getBlock()) {
            ...
            if(tileEntity instanceof FluidMixerTileEntity) {
                ItemStackHandler inventory = ((FluidMixerTileEntity)tileEntity).inventory;
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if(!worldIn.isRemote) {
            TileEntity tileEntity = worldIn.getTileEntity(pos);
            if(tileEntity instanceof FluidMixerTileEntity)
                NetworkHooks.openGui((ServerPlayerEntity)player, (FluidMixerTileEntity)tileEntity, pos);
        }
    ...
    public int getComparatorInputOverride(BlockState blockState, World worldIn, BlockPos pos) {
        TileEntity tileEntity = worldIn.getTileEntity(pos);
        if(tileEntity instanceof FluidMixerTileEntity)
            return ItemHandlerHelper.calcRedstoneFromInventory(((FluidMixerTileEntity)tileEntity).inventory);
